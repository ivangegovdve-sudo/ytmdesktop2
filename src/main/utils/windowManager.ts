...
export function isPreventedNavOrRedirect(url: URL): boolean {
  return (
    /^(?!consent\.youtube\.com|accounts\.youtube\.com|music\.youtube\.com|accounts\.google\.\w+).*$/.test(
      url.hostname,
    ) &&
    !/^(www\.youtube\.com|youtube\.com)\/(premium|musicpremium)$/i.test(
...
const GOOGLE_LOGIN_URL =
  ...
  new URLSearchParams({
    ...
    service: "youtube",
    continue:
      "https://www.youtube.com/signin?action_handle_signin=true&app=desktop&next=https://music.youtube.com/",
  }).toString();
...
export class WindowManager {
  ...
  private setupYoutubeViewEvents(view: WebContentsView) {
    ...
    view.webContents.on("page-title-updated", (ev, title) =>
      view.webContents.emit("window-title-updated", title),
    );
  }
  ...
  private setupViewEvents() {
    ...
    const handleLoadStart = debounce(() => {
      if (!this.mainWindow || !this.loadingView || !this.views) return;
      if (
        !this.mainWindow.contentView.children?.find(
          (x) => this.loadingView?.webContents && x === this.loadingView,
        )
      ) {
        this.mainWindow.contentView.addChildView(this.loadingView);
      }
