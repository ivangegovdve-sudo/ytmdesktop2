import {
  ...
  DiscordActivityType,
  type DiscordActivity as Presence,
} from "@main/lib/discord-rpc/discord-rpc";
...
export const parseMusicAlbumById = (ident: string) =>
  `https://music.youtube.com/browse/${encodeURIComponent(ident)}?feature=share`;
export const discordEmbedFromTrack = (
  ...
  const startDate = playing ? new Date(Date.now() - progress * 1000) : undefined,
    endDate = startDate
      ? new Date(startDate.getTime() + ~~Number(track.video.lengthSeconds) * 1000)
      : undefined;
  ...
  const authorUrl = track.video.channelId
    ? parseMusicChannelById(track.video.channelId)
    : undefined;
