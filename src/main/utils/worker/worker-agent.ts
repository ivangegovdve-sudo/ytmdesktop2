...
export class WorkerAgent<TInput, TOutput> {
  ...
  on(event: "result", callback: (err: Error | null, output: TOutput | null) => void): this;
  ...
  once(event: "result", callback: (err: Error | null, output: TOutput | null) => void): this;
  ...
  private registerResultListener(
    ...
    workerSubscriptionMethod.call(this.worker, "message", (message: MessageFromWorker) => {
      switch (message.type) {
