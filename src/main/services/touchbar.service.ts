    ...
    try {
      ...
      trackProvider.onTrackStateChange(
        (state) => {
          ...
          dislikeButton.backgroundColor = state.disliked
            ? emoteColors.dislike
            : emoteColorsOff.dislike;
      ...
      trackProvider.onTrackChange(async (track) => {
        ...
        this.logger.debug(
          "TouchbarProvider onTrackChange",
          songTitle.label,
          track.video.thumbnail.thumbnails?.[0]?.url,
        );
        const buffer =
          track.video.thumbnail.thumbnails?.[0]?.url &&
          (await fetch(track.video.thumbnail.thumbnails[0].url).catch(() => null));
        songImage.icon = (
          buffer
            ? nativeImage.createFromBuffer(Buffer.from(await buffer.arrayBuffer()))
