...
export default class CustomCSSProvider extends BaseProvider implements AfterInit {
  ...
  private async injectCompiledCSS(
    scssPath: string,
    bypassCache: boolean = false,
  ): Promise<boolean> {
  ...
  private async updateCSS(bypassCache: boolean = false) {
    ...
    this.logger.debug(
      `Loading custom CSS from ${scssPath}${bypassCache ? " (cache bypassed)" : ""}`,
    );
  ...
  @IpcOn(IPC_EVENT_NAMES.SERVER_SETTINGS_CHANGE, {
    filter: (key: string) =>
      ["customcss.enabled", "customcss.scssFile", "customcss.watching"].includes(key),
    debounce: 1000,
  })
  ...
  private async _event_settingsChangeThumbnailBackground(
    _key: string,
    thumbnailBackgroundEnabled: boolean,
  ) {
    if (!this.thumbnailBackgroundStyle)
      this.thumbnailBackgroundStyle = new CSSHandler(
        this.windowContext.views.youtubeView.webContents,
        { translateSass: true },
      );
    if (thumbnailBackgroundEnabled && !this.thumbnailBackgroundStyle.isCreated)
      await this.thumbnailBackgroundStyle.createOrUpdate(playerThumbnailStyle);
  ...
  private attachBasicStyle() {
    ...
    const thumbnailBackgroundEnabled =
      this.settingsInstance.instance.customcss?.thumbnailBackground ?? true;
    if (thumbnailBackgroundEnabled)
      this.thumbnailBackgroundStyle.createOrUpdate(playerThumbnailStyle);

    this.logger.debug("basicStyleContent", {
      thumbnail: playerThumbnailStyle,
      basic: basicScrollStyle,
    });
  }
