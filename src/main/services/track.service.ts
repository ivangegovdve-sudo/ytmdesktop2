...
export default class TrackProvider extends BaseProvider implements AfterInit {
  ...
  setTrackState(fn: TrackState | ((d: TrackState) => void | TrackState)) {
    ...
    if (typeof this.trackState?.percentage === "number")
      this.trackState.percentage = clamp(this.trackState.percentage, 0, 100);
  ...
  async getActiveTrackByDOM(): Promise<string | null> {
    try {
      const videoId = await this.views.youtubeView.webContents.executeJavaScript(`
				(() => {
					const href = document.querySelector("a.ytp-title-link.yt-uix-sessionlink")?.href;
					if (!href) return null;
  ...
  onTrackStateChange(
    callback: (state: TrackState) => void,
    options: { debounce?: number; immediate?: boolean } = { immediate: false },
  ) {
  ...
  onTrackChange(
    ...
    options: { debounce?: number; immediate?: boolean } = { debounce: 1000, immediate: false },
  ) {
    const handler = debounce(callback, options?.debounce);
  ...
  private handleTrackStyle() {
    ...
    this.onTrackChange(async (track) => {
      ...
      this.windowContext.views.youtubeView.webContents.send(
        "css.thumbnail-accent",
        trackAccent ?? "transparent",
      );
      this.logger.debug("track:accent", trackAccent, track.video.thumbnail.thumbnails?.[0]?.url);
    });
    this.onTrackChange(async (track) => {
      const hqThumbnail =
        track.context?.thumbnail?.thumbnails?.sort(firstBy((d) => d.width, "desc"))[0]?.url ??
        track.meta.thumbnail;
