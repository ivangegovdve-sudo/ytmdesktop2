...
export default function definePlugin(
  name: string,
  options: Omit<PluginOptions, "name"> = { enabled: true, displayName: name },
  fn: PluginExec,
): ClientPlugin {
...
export const pluginCommandKeySlug = (cmd: string) =>
  cmd.replace(/\.?(?=[A-Z])/g, "_").toLowerCase();
/**
 ...
 * @returns void
 */
export function initializePluginCommandsWithIPC(
  plugin: ClientPlugin,
  ...
  Object.entries(cmds).forEach(([cmd, fn]) => {
    ...
    const handler = async (
      ev: unknown,
      { requestId, payload }: { requestId: string; payload: any[] },
    ) => {
      pluginContext.log.debug(`Received command \`${cmd}\` with IPC`, { requestId, payload });
      const response = await Promise.resolve(
        fn(pluginContext, ...(Array.isArray(payload) ? payload : [payload])),
      );
      window.ipcRenderer.send(
        `plugins:${pluginName}:cmd:${commandKey}/response.${requestId}`,
        requestId,
        response,
      );
      pluginContext.log.debug(`Sent response for command \`${cmd}\` with IPC`, {
        requestId,
        response,
      });
    };
