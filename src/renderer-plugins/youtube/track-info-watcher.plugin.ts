    ...
    afterInit({ domUtils, playerApi, playerUiService, api }) {
      ...
      domUtils.ensureDomLoaded(() => {
        const handleVideoDataChange = (ev: any) => {
          ...
          let currentItem = document.querySelector<any>(
            "ytmusic-app-layout>ytmusic-player-bar",
          )?.currentItem;
          if (currentItem !== null && currentItem !== undefined) {
            const albumRef = currentItem.longBylineText.runs.find(
              (v: any) =>
                v.navigationEndpoint?.browseEndpoint?.browseEndpointContextSupportedConfigs
          ...
          const requestData = {
            ...
            context:
              (videoData.microformat ? videoData.microformat.microformatDataRenderer : null) ||
              null,
            music:
