...
export default definePlugin(
  ...
  ({ domUtils }) => {
    ...
    async function handleThumbnail(_ev, value: string) {
      ...
      if (value)
        destroyStyle = await domUtils.createStyle(`:root { --ytmd-thumbnail-url: ${value}; }`); // ! todo: add working css for track change watcher
    }
    let destroyAccent: () => void;
    async function handleAccent(_ev, value: string) {
      if (destroyAccent) destroyAccent();
      if (value)
        destroyAccent = await domUtils.createStyle(`:root { --ytmd-thumbnail-accent: ${value}; }`); // ! todo: add working css for track change watcher
    }
