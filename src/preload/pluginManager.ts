...
export class PluginManager {
  ...
  private pluginMap: Map<string, PluginInfo> = new Map();
  ...
  constructor() {
    this.settingsPromise = window.api.settingsProvider
      .getAll({})
  ...
  private loadPlugins(): void {
    ...
    this.plugins = Object.entries(pluginModules)
      .map(([filename, p]: [string, any]) => {
        ...
        const { meta, exec, afterInit, cmds } = p.default as ClientPlugin;
      ...
      .filter(
        (p): p is NonNullable<typeof p> => p !== undefined && p.meta && p.meta.enabled !== false,
      );

    // Expose plugins globally
    window.__ytd_plugins = Object.freeze(this.plugins);
    this.pluginMap = new Map(this.plugins.map((p) => [p.name, p]));
  }
  ...
  private async initializePlugins(): Promise<void> {
    ...
    const results = await Promise.all(
      this.plugins.map(async (plugin) => {
        ...
        const result = await Promise.resolve(
          plugin.exec({ ...pluginContext, log: plugin.log, playerApi: this.getPlayerApi() }),
        );
        return result;
      }),
    );
    ...
    window.addEventListener("beforeunload", async () => {
      ...
      if (
        this.destroyFns &&
        currentUrl.hostname !== location.hostname &&
    ...
    await Promise.all(
      this.plugins.map(async (plugin) => {
        ...
        await Promise.resolve(
          plugin.afterInit({ ...pluginContext, log: plugin.log, playerApi: this.getPlayerApi() }),
        );
    ...
    await Promise.all(
      this.plugins.map((plugin) => {
        ...
        return Promise.resolve(
          initializePluginCommandsWithIPC(clientPlugin, {
            ...pluginContext,
            log: plugin.log,
  ...
  public async initialize(force?: boolean): Promise<void> {
    ...
    await this.removeChromecastIcon().catch((ex) =>
      this.log.error("removeChromecastIcon failed", ex),
    );
