...
const createPlayerApi = () => {
  ...
  return () =>
    playerApiCache ||
    (playerApiCache = (document.querySelector("body>ytmusic-app") as any)?.playerApi);
};

const createPlayerUiService = () => {
  let playerUiServiceCache: any;
  return () =>
    playerUiServiceCache ||
    (playerUiServiceCache = (document.querySelector("body>ytmusic-app") as any)?.playerUiService);
};
...
export default {
  ...
  domUtils: {
    ...
    awaitElement: <T extends HTMLElement>(
      selector: string,
