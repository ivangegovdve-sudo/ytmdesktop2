      ...
      if (window.trustedTypes?.defaultPolicy?.name === "default") {
        window.trustedTypes.createPolicy("default", {
          createHTML: (string: string) =>
            DOMPurify.sanitize(string, { RETURN_TRUSTED_TYPE: true }) as any,
...
export const createPluginUtils = (): PluginUtils => ({
  ...
  createPluginLogger: (baseLogger: any, pluginName: string) =>
    baseLogger.child(`Client Plugin, ${pluginName}`),
...
export const createHostDetector = (prefix: string) => () =>
  window && document.location.host.indexOf(prefix) === 0;
