  ...
  >
    ...
    <div class="flex items-center space-x-2">
      ...
      <div class="flex items-center space-x-1">
        <template v-if="!isMac">
          <button v-if="state?.minimizable" class="control-button" @click="onMin" aria-label="Minimize">
            <MinIcon />
          </button>
          <button v-if="state?.maximizable" class="control-button" @click="onMax" aria-label="Maximize">
            <MaxIcon />
          </button>
        </template>
        <button v-if="showClose" class="control-button control-button-danger" @click="onClose" aria-label="Close">
          <CloseIcon />
        </button>
      </div>
    </div>
