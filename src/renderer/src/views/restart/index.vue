<template>
  <div class="min-h-screen flex flex-col p-4 bg-black overflow-hidden h-full">
    ...
    <div
      class="text-center py-4 px-6 flex items-center justify-center flex-col flex-grow gap-4 mb-6"
    >
    ...
    <div class="flex gap-3 pt-2">
      ...
      <button
        @click="action('ok')"
        :disabled="isBusy"
        class="w-full px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md transition-colors flex items-center justify-center gap-2"
      >
        Restart
      </button>
    </div>
...
const meta = computed(() => {
  ...
  const message =
    params.get("message") ?? "Please restart the application to apply pending changes.";
  return {
    message,
    icon,
  };
