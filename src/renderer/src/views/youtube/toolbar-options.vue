  ...
  <div class="flex flex-row items-center space-x-2">
    ...
    >
      <template v-if="lastFM.connected && !lastFM.error && lastFMState !== null">
        <Spinner v-if="typeof lastFMState === 'string'" size="sm" />
    ...
    <button
      ...
      @click="() => checkUpdate()"
    >
      <Spinner v-if="updateChecking && !updateInfo" size="sm" />
      <DownloadIcon
        v-else-if="updateInfo"
        :class="{ 'animate-pulse': !updateDownloaded, 'text-green-500': updateDownloaded }"
      ></DownloadIcon>
    ...
    >
      <RPCIcon
        :class="{
          'text-red-500': discordConnectionError && discordEnabled,
          'opacity-100': discordEnabled || discordConnectionError,
          'opacity-70': !discordEnabled || discordLoading,
        }"
      ></RPCIcon>
      <div
        v-if="discordConnected && !discordConnectionError && !discordLoading"
...
const discordConnectionMap = {
  ["discord.connected"]: true,
  ["discord.disconnected"]: false,
} as const;
