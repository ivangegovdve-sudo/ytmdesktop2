<template>
  <div class="min-h-screen flex flex-col p-4 bg-black overflow-y-auto overflow-x-hidden h-full">
    ...
    <div v-else-if="updateInfo">
      ...
      <div class="text-center pb-4 pt-6 px-6 drag">
        <div class="mx-auto mb-4 p-3 rounded-full bg-gray-900 w-fit">
          <DownloadIcon :size="32" class="text-blue-400" />
        </div>
      ...
      <div class="px-6 pb-6 space-y-6">
        ...
        <div class="space-y-3">
          ...
          <template v-if="updateInfo.releaseNotes">
            <MDXProvider :components="MDXComponents">
              <div class="text-sm text-gray-200" v-html="updateInfo.releaseNotes" />
            </MDXProvider>
          </template>
        </div>
        <!-- Download Progress -->
        <div v-if="updateInfoProgress" class="space-y-3">
          <div class="border-t border-gray-800"></div>
          <div class="space-y-2">
            ...
            <div class="w-full bg-gray-800 rounded-full h-2">
              <div
                ...
                :style="{ width: `${updateInfoProgress.percent}%` }"
              ></div>
            </div>
        ...
        <div class="flex gap-3 pt-2">
          <template v-if="updateInfoProgress && !updateDownloaded">
            <button
              class="w-full px-4 py-2 border border-gray-700 text-gray-300 rounded-md opacity-50 cursor-not-allowed"
              disabled
            >
              Downloading...
            </button>
          ...
          <template v-else-if="updateDownloaded && !isInstalling">
            <button
              @click="window.close()"
              class="flex-1 px-4 py-2 border border-gray-700 text-gray-300 hover:bg-gray-800 hover:text-white rounded-md transition-colors"
            >
              Later
            </button>
            <button
              @click="installUpdate(true)"
    ...
    <div v-else class="text-center py-12 px-6 flex items-center justify-center flex-col flex-grow">
      <div class="mx-auto mb-4 p-3 rounded-full bg-gray-900 w-fit">
        <CheckCircleIcon :size="32" class="text-green-400" />
      </div>
      <h2 class="text-white text-xl font-semibold mb-2">You're Up to Date</h2>
      <p class="text-gray-400 text-sm mb-6">
        Your software is running the latest version ({{ currentVersion }})
      </p>
      <div class="flex flex-col gap-2 items-center">
        <button class="btn btn-sm" @click="checkUpdate">Check Again</button>
        <button class="btn btn-ghost btn-sm" @click="window.close()">Close</button>
      </div>
    </div>
  </div>
</template>
...
const [updateInfoProgress, setUpdateInfoProgress] = refIpc<ProgressInfo | null>(
  "APP_UPDATE_PROGRESS",
  {
