...
export function refIpc<T, R = T>(
  ...
  const { defaultValue, mapper, onTrigger, ignoreUndefined, rawArgs, getInitialValue } =
    options ?? {};
  ...
  const handlers: { [key: string]: IpcHandler } = handlerNames.reduce((acc, handlerName) => {
    acc[handlerName] = ((ev, ...data) => {
      ...
      // Prevent Vue component re-renders if the data structure hasn't actually changed.
      // IPC frequently delivers structurally identical objects with new memory references.
      if (isEqual(state.value, newVal)) return;
