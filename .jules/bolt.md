...
## 2026-03-26 - O(N^2) String Regex parsing inside Array.prototype.reduce
**Learning:** When iteratively constructing or modifying strings character-by-character, avoid running `String.prototype.replace()` with regular expressions inside an `Array.prototype.reduce()` loop. This pattern causes O(N^2) complexity due to repeated regex parsing and string allocations on every iteration.
**Action:** Optimize by hoisting the regex initialization, using `.map()` to build a character array, `.join("")`ing it, and running a single `.replace()` on the final string.
